...
df["combined_features"] = df.apply(combine_features, axis=1)  #multiple features combined into one feature

from sklearn.feature_extraction.text import CountVectorizer
from sklearn.metrics.pairwise import cosine_similarity

cv = CountVectorizer()   #method to convert text to numerical data
count_matrix = cv.fit_transform(df["combined_features"])

cosine_sim = cosine_similarity(count_matrix)
# print (cosine_sim)
cosine_sim.shape    #Getting a 1 x N similarity matrix instead of N x N one using Count Vectorizer
